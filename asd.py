...
def lista_pododdzialow(listbox_lista_pododdzialy):
    listbox_lista_pododdzialy.delete(0, END)
    for idx, pododdzial in enumerate(pododdzialy):
        listbox_lista_pododdzialy.insert(idx,
...
def dodaj_pododdzial(entry_nazwa_pododdzialu, entry_nazwa_jednostki, entry_pracownicy, entry_lokalizacja_pododdzialu,
    ...
    print(nazwa_pododdzialu, nazwa_jednostki, pracownicy, lokalizacja_pododdzialu)
    pododdzialy.append(Pododdzial(nazwa_pododdzialu, nazwa_jednostki, pracownicy, lokalizacja_pododdzialu))
    lista_pododdzialow(listbox_lista_pododdzialy)
...
def usun_pododdzial(listbox_lista_pododdzialy):
    i = listbox_lista_pododdzialy.index(ACTIVE)
    print(i)
    pododdzialy.pop(i)
    lista_pododdzialow(listbox_lista_pododdzialy)


def pokaz_szczegoly_pododdzialu(listbox_lista_pododdzialy, label_nazwa_pododdzialu_szczegoly_obiektu_wartosc,
    ...
    label_lokalizacja_pododdzialu_szczegoly_obiektu_wartosc.config(text=lokalizacja_pododdzialu)


def edytuj_pododdzial(listbox_lista_pododdzialy, entry_nazwa_pododdzialu, entry_nazwa_jednostki, entry_pracownicy,
    ...
    button_dodaj_pododdzial.config(command=lambda: aktualizuj_pododdzial(i, entry_nazwa_jednostki, entry_pracownicy,
                                                                         entry_lokalizacja_pododdzialu,
                                                                         button_dodaj_pododdzial,
                                                                         listbox_lista_pododdzialy))
...
def aktualizuj_pododdzial(i, entry_nazwa_pododdzialu, entry_nazwa_jednostki, entry_pracownicy,
                          entry_lokalizacja_pododdzialu, button_dodaj_uzytkownia,
                          listbox_lista_pododdzialy):
    pododdzialy[i].nazwa_pododdzialu = entry_nazwa_pododdzialu.get()
    pododdzialy[i].nazwisko = entry_nazwa_jednostki.get()
    pododdzialy[i].pracownicy = entry_pracownicy.get()
    pododdzialy[i].lokalizacja_pododdzialu = entry_lokalizacja_pododdzialu.get()
    lista_pododdzialow(listbox_lista_pododdzialy)
    button_dodaj_uzytkownia.config(text="Dodaj uzytkownikow",
                                   command=lambda: dodaj_pododdzial(entry_nazwa_pododdzialu.get(),
                                                                    entry_nazwa_jednostki.get(), entry_pracownicy.get(),
                                                                    entry_lokalizacja_pododdzialu.get(),
                                                                    listbox_lista_pododdzialy))
    entry_nazwa_pododdzialu.delete(0, END)
    entry_nazwa_jednostki.delete(0, END)
    entry_pracownicy.delete(0, END)
...
def tworzenie_pododdzialy_root(root):
    ...
    button_edytuj_obiektu = Button(ramka_lista_pododdzialy, text='Edytuj obiekt',
                                   command=lambda: edytuj_pododdzial(listbox_lista_pododdzialy, entry_nazwa_pododdzialu,
                                                                     entry_nazwa_jednostki, entry_pracownicy,
                                                                     entry_lokalizacja_pododdzialu,
                                                                     button_dodaj_pododdzial))
    ...
    button_dodaj_pododdzial = Button(ramka_formularz, text="Dodaj pododdzial",
