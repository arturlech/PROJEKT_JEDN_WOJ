...
def lista_uzytkownikow(listbox_lista_pracownikow):
    ...
    for idx, pracownik in enumerate(pracownicy):
        listbox_lista_pracownikow.insert(idx, f'{pracownik.imie}  {pracownik.nazwisko} {pracownik.stanowisko} {pracownik.lokalizacja}')
...
def pokaz_szczegoly_uzytkownikow(listbox_lista_pracownikow, label_imie_szczegoly_obiektu_wartosc, label_nazwisko_szczegoly_obiektu_wartosc,label_stanowisko_szczegoly_obiektu_wartosc,label_lokalizacja_szczegoly_obiektu_wartosc):
    i = listbox_lista_pracownikow.index(ACTIVE)
...
def edytuj_uzytkownika(listbox_lista_pracownikow, entry_imie, entry_nazwisko, entry_stanowisko, entry_lokalizacja, button_dodaj_uzytkownika):
    i = listbox_lista_pracownikow.index(ACTIVE)
    ...
    button_dodaj_uzytkownika.config(text="Zapisz zmiany", command=lambda: aktualizauj_uzytkownika(i, entry_imie, entry_nazwisko, entry_stanowisko, entry_lokalizacja, button_dodaj_uzytkownika, listbox_lista_pracownikow))


def aktualizauj_uzytkownika(i, entry_imie, entry_nazwisko, entry_stanowisko, entry_lokalizacja, button_dodaj_uzytkownia, listbox_lista_pracownikow):
...
def tworzenie_pracownicy_root(root):
    ...
    button_pokaz_szczegoly = Button(ramka_lista_pracownikow, text="Pokaż szczegóły", command=lambda: pokaz_szczegoly_uzytkownikow(listbox_lista_pracownikow, label_imie_szczegoly_obiektu_wartosc, label_nazwisko_szczegoly_obiektu_wartosc,label_stanowisko_szczegoly_obiektu_wartosc,label_lokalizacja_szczegoly_obiektu_wartosc))
    button_usun_obiekt = Button(ramka_lista_pracownikow, text='Usuń obiekt', command=lambda: usun_uzytkownika(listbox_lista_pracownikow))
    button_edytuj_obiektu = Button(ramka_lista_pracownikow, text='Edytuj obiekt', command=lambda: edytuj_uzytkownika(listbox_lista_pracownikow, entry_imie, entry_nazwisko, entry_stanowisko, entry_lokalizacja, button_dodaj_uzytkownia))
    ...
    button_dodaj_uzytkownia = Button(ramka_formularz, text="Dodaj użytkownika", command=lambda: dodaj_uzytkownika(entry_imie, entry_nazwisko, entry_stanowisko, entry_lokalizacja, listbox_lista_pracownikow))
...
def tworzenie_jednostki_wojskowe_root(root):
    ...
    # ramki do porządkowania struktury
    ramka_lista_pracownikow = Frame(jednostki_wojskowe_root)
    ...
    label_szczegoly_obiektu = Label(ramka_szczegoly_obiektu, text="Szczegóły użytkownika: ")
    label_nazwa_szczegoly_obiektu = Label(ramka_szczegoly_obiektu, text="Nazwa jednostki: ")
    label_funkcja_szczegoly_obiektu = Label(ramka_szczegoly_obiektu, text="Funkcja: ")
    ...
    label_funkcja_szczegoly_obiektu_wartosc = Label(ramka_szczegoly_obiektu, text="...")
    ...
    label_funkcja_szczegoly_obiektu.grid(row=1, column=2)
    label_funkcja_szczegoly_obiektu_wartosc.grid(row=1, column=3)
...
def tworzenie_pododdzialy_root(root):
        ...
        # ramki do porządkowania struktury
        ramka_lista_pracownikow = Frame(pododdzialy_root)
...
def logowanie():
    while True:
        ...
        if (login == login_1 and haslo == haslo_1) or \
           ...
           (login == login_4 and haslo == haslo_4):
